 ...
 */
namespace Opencart\Admin\Controller\Extension\Mcp\Module;

class Mcp extends \Opencart\System\Engine\Controller {
    private $error = array();

    public function index() {
        $this->load->language('extension/mcp/module/mcp');
        ...
        if (($this->request->server['REQUEST_METHOD'] ?? 'GET') === 'POST' && $this->validate()) {
            ...
            $this->response->redirect($this->url->link('extension/mcp/module/mcp', 'user_token=' . $this->session->data['user_token']));
            return;
        }
        ...
        $data['clients'] = $this->model_extension_mcp_module_mcp->listClients();
        $data['audit_logs'] = $this->model_extension_mcp_module_mcp->listAudit(50, $auditFilters);
        $data['audit_filters'] = $auditFilters;
        $data['audit_filter_query'] = $auditFilters ? '&' . http_build_query($auditFilters) : '';
        $data['capability_packs'] = $this->model_extension_mcp_module_mcp->capabilityPacks();
        $data['tools_by_capability'] = $this->model_extension_mcp_module_mcp->toolsByCapability();
        ...
        $this->response->setOutput($this->load->view('extension/mcp/module/mcp', $data));
    }
    ...
    public function revoke() {
        $this->load->model('extension/mcp/module/mcp');
        if ($this->validate()) {
            $this->model_extension_mcp_module_mcp->revokeClient((int)($this->request->get['client_id'] ?? 0));
    ...
    public function reviewAudit() {
        ...
        if ($this->validate()) {
            $this->model_extension_mcp_module_mcp->markAuditReviewed((int)($this->request->post['audit_id'] ?? 0), $this->request->post['admin_note'] ?? '');
    ...
    public function auditDetail() {
        ...
        $this->response->setOutput(json_encode($this->model_extension_mcp_module_mcp->getAudit((int)($this->request->get['audit_id'] ?? 0)) ?: array()));
    }

    public function exportAudit() {
        $this->load->model('extension/mcp/module/mcp');
        $rows = $this->model_extension_mcp_module_mcp->auditCsvRows($this->auditFilters(), 1000);
    ...
    public function install() {
        $this->load->model('extension/mcp/module/mcp');
        $this->model_extension_mcp_module_mcp->install();
        $this->load->model('user/user_group');
        $group_id = $this->user->getGroupId();
        $this->model_user_user_group->addPermission($group_id, 'access', 'extension/mcp/module/mcp');
        $this->model_user_user_group->addPermission($group_id, 'modify', 'extension/mcp/module/mcp');
    }

    public function uninstall() {
        $this->load->model('extension/mcp/module/mcp');
        $this->model_extension_mcp_module_mcp->uninstall();
    }
    ...
    private function baseData() {
        ...
        $data['breadcrumbs'] = array(
            array('text' => $this->language->get('text_home'), 'href' => $this->url->link('common/dashboard', $token)),
            array('text' => $this->language->get('text_extension'), 'href' => $this->url->link('marketplace/extension', $token . '&type=module')),
            array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('extension/mcp/module/mcp', $token)),
        );
        $data['action'] = $this->url->link('extension/mcp/module/mcp', $token);
        $data['create_client'] = $this->url->link('extension/mcp/module/mcp.createClient', $token);
        $data['revoke_client'] = $this->url->link('extension/mcp/module/mcp.revoke', $token);
        $data['audit_export'] = $this->url->link('extension/mcp/module/mcp.exportAudit', $token);
        $data['audit_detail'] = $this->url->link('extension/mcp/module/mcp.auditDetail', $token);
        $data['audit_review'] = $this->url->link('extension/mcp/module/mcp.reviewAudit', $token);
    ...
    private function validate() {
        if (!$this->user->hasPermission('modify', 'extension/mcp/module/mcp')) {
